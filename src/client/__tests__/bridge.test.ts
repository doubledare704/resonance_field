...
import { NodeType, ResponseType } from '../../shared/api';
...
describe('requestInitialSnapshot', () => {
  it('returns ok: true on success', async () => {
    mockResponse({ type: ResponseType.Snapshot, snapshot: {}, contractVersion: 'v1' });
    const result = await requestInitialSnapshot();
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.data.type).toBe(ResponseType.Snapshot);
    }
  });

  it('returns ok: false on HTTP error', async () => {
    mockResponse({ type: ResponseType.Error, message: 'fail', contractVersion: 'v1' }, false, 400);
    const result = await requestInitialSnapshot();
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error.type).toBe(ResponseType.Error);
    }
  });
...
describe('deployNodeRequest', () => {
  it('sends POST to /api/node-deploy with node data', async () => {
    mockResponse({ type: ResponseType.NodeDeployed, node: {}, snapshot: {}, removedNodeId: null, contractVersion: 'v1' });
  ...
  it('returns error on failure', async () => {
    mockResponse({ type: ResponseType.Error, message: 'bad', contractVersion: 'v1' }, false, 400);
...
describe('submitThroughputRequest', () => {
  it('sends POST to /api/throughput with count', async () => {
    mockResponse({ type: ResponseType.ThroughputAccepted, snapshot: {}, scoreDelta: 5, contractVersion: 'v1' });
...
describe('resetDailyStateRequest', () => {
  it('sends POST to /api/reset', async () => {
    mockResponse({ type: ResponseType.ResetComplete, snapshot: {}, archivedScore: 100, contractVersion: 'v1' });
