...
- `initial.md` is the canonical product and architecture prompt.
- Use Phaser 3 in the client and Devvit as the server authority.
- Preserve the cooperative fluid sandbox, trilogy rule, 60 second node expiry, and daily UTC reset.
- Keep realtime payloads compact and the UI neon, geometric, and minimal.
- Prefer the local routing files in `agents.md`, `hooks/`, and `skills/` when planning work.
